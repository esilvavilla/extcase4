...
#POSTGRES_ADDRESS = config.my_host
#POSTGRES_PORT = config.my_port
#POSTGRES_USERNAME = config.my_user
#POSTGRES_PASSWORD = config.my_passwd 
#POSTGRES_DBNAME = config.my_app_python
...
def update_dates_exchange(value):
    okexch = df['Exchange'] == value
    return ( [df['Entry time'][okexch].min(), df['Entry time'][okexch].max()] )

def filter_df(exchange,leverage,start_date,end_date):
    dff = df.loc[ (df['Exchange'] == exchange) & (df['Margin'] == leverage) & (df['Entry time'] >= start_date) & (df['Entry time'] < end_date) ]
...
def update_figure(exchange,leverage,start_date,end_date):
        dff = filter_df(exchange,leverage,start_date,end_date)

        return ({ 
                'data': [
                    go.Candlestick(x=dff['Entry time'],
                               open=dff['Entry balance'],
                               high=dff['Exit balance'],
                               low=dff['Entry balance'],
...
def update_figure(exchange,leverage,start_date,end_date):
        ...
        return ({ 
                'data': [go.Bar( x=dff['Entry time'], y=dff['Pnl (incl fees)'])],
                'layout': go.Layout(title=f"PnL vs Trade type",height= 500, colorway=["#EF963B", "#EF533B"]) 
        }) 


@app.callback(
    dash.dependencies.Output('daily-btc', 'figure'), # component with id map-plot will be changed, the 'figure' argument is updated
    [
        dash.dependencies.Input('exchange-select', 'value'),
        dash.dependencies.Input('leverage-select', 'value'),
        dash.dependencies.Input('date-range', 'start_date'),
...
def update_btc_figure(exchange,leverage,start_date, end_date):
    dff = filter_df(exchange,leverage,start_date, end_date)
...
@app.callback(
    dash.dependencies.Output('balance', 'figure'), # component with id map-plot will be changed, the 'figure' argument is updated
    [
        dash.dependencies.Input('exchange-select', 'value'),
        dash.dependencies.Input('leverage-select', 'value'),
        dash.dependencies.Input('date-range', 'start_date'),
...
def update_ptb_figure(exchange,leverage,start_date, end_date):
    dff = filter_df(exchange,leverage,start_date, end_date)

    return ({ 
        'data': [go.Scatter(x=dff['Entry time'], y= (dff['Exit balance']-dff['Entry balance'])  )],
        'layout': go.Layout(title='Balance over time',height= 400)
    })
